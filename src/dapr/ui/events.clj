...
(defonce ^:private state-atom*
  ;; The live state atom, exposed to the log window's raw JavaFX scrollTop listener —
  ;; something cljfx can't express as a prop, and which is attached from a pure view
  ;; fn that has no handle to the atom. Set by make-handler. The declarative
  ;; {:event/type …} handlers need no such bridge: cljfx routes those through the
  ;; map-event-handler make-handler returns.
  (atom nil))
...
(defn on-log-scroll!
  "Feed a live-log TextArea scrollTop change (`pos`) into follow/freeze state. Called
  by the window's raw scrollTop ChangeListener (see dapr.ui.views). Since
  state/log-scroll-changed ignores the near-zero reset a text replace produces, the
  programmatic pin never disengages following."
  [pos]
  (when-let [sa @state-atom*]
    (swap! sa state/log-scroll-changed pos)))
